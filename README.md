# Open Protector Standard
An open-source back protector standard for paragliding harnesses.

I recommend reading in the following order:

1. [Problems and solutions](./problems_solutions.md)
2. [OPS2025 standard](./standard/OPS2025.md)
3. [Questions](./questions.md)
4. [Work In Progress](./work_in_progress.md)

## License

[![CC0 logo](https://mirrors.creativecommons.org/presskit/buttons/88x31/svg/cc-zero.svg)](https://creativecommons.org/publicdomain/zero/1.0/)

We believe safety standards should be freely accessible - not locked behind paywalls.

To maximize reuse and adoption in safety regulations, the **text content** of this standard is dedicated to the public domain under [CC0 1.0 Universal](https://creativecommons.org/publicdomain/zero/1.0/).

> CC0 enables scientists, educators, and creators to waive interests in their works and thereby place them as completely as possible in the public domain, so that others may freely build upon, enhance and reuse the works for any purposes without restriction.

**Note:** This waiver applies to the specifications and text only. It does **not** apply to images in the `assets/` folder, which remain under their respective third-party copyrights.

